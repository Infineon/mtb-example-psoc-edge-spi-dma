...
The following example clones the "[SPI with DMA](https://github.com/Infineon/mtb-example-psoc-edge-spi-dma)" application with the desired name "SpiDma" configured for the *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC2 --app-id mtb-example-psoc-edge-spi-dma --user-app-name SpiDma --target-dir "C:/mtb_projects"
   ```
